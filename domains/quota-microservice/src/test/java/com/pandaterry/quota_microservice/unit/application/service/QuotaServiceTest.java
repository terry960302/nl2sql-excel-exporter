...
class QuotaServiceTest {
    ...
    void getQuotaForOrg_성공() {
        ...
        when(dailyRepository.findById(new QuotaUsageDailyKey(orgId, today))).thenReturn(Optional.of(daily));
        when(monthlyRepository.findById(new QuotaUsageMonthlyKey(orgId, monthKey))).thenReturn(Optional.of(monthly));
    ...
    void recordQuotaUsage_신규레코드생성() {
        LocalDate today = LocalDate.now();
        String monthKey = YearMonth.now().format(DateTimeFormatter.ofPattern("yyyyMM"));
        when(dailyRepository.findById(new QuotaUsageDailyKey(orgId, today))).thenReturn(Optional.empty());
        when(monthlyRepository.findById(new QuotaUsageMonthlyKey(orgId, monthKey))).thenReturn(Optional.empty());
